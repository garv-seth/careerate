...
// Type definitions for company, role, and level
interface Company {
  id: string;
  name: string;
}
...
const TransitionForm: React.FC = () => {
  ...
  const { data: currentRolesData } = useQuery<QueryResponse<Role>>({
  ...
  const { data: targetRolesData } = useQuery<QueryResponse<Role>>({
  ...
  const { data: currentLevelsData } = useQuery<QueryResponse<Level>>({
  ...
  const { data: targetLevelsData } = useQuery<QueryResponse<Level>>({
